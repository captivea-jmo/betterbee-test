...
class DeliveryCarrier(models.Model):
    ...
    delivery_type = fields.Selection(selection_add=[('easypost_ts', "Easypost")], ondelete={'easypost_ts': 'cascade'})
    ...
    @api.model
    def easypost_ts_add_parcel(self, weight, package=False):
        parcel_dict = {"parcel": {'weight': weight}, 'options': {'label_size': "4X6", 'label_format': self.ep_label_file_type}}
        if package:
            ...
            # Modif Captivea #
            ##################
                # add custom parcel dimension height, width and length here from stock.quant.package
                # parcel_dict['parcel'].update({'length': package.packaging_id.packaging_length, 'width': package.packaging_id.width, 'height': package.packaging_id.height})
                parcel_dict['parcel'].update({'length': package.length, 'width': package.width,'height': package.height})
            ## FIN Modif #####
            return parcel_dict
        if self.ep_product_packaging_id.shipper_package_code:
            parcel_dict['parcel'].update({'predefined_package': self.ep_product_packaging_id.shipper_package_code})
        else:
            parcel_dict['parcel'].update({'length': self.ep_product_packaging_id.packaging_length, 'width': self.ep_product_packaging_id.width, 'height': self.ep_product_packaging_id.height})
        return parcel_dict
            ...
            try:
                ...
                if picking.package_ids:
                    ...
                    for package in picking.package_ids:
                        ...
                        parcel_dict = self.easypost_ts_add_parcel(package_weight, package=package)
                        if is_int_shipment:
                ...
                buy_response = self.shipping_partner_id._easypost_send_request('orders/%s/buy' % response['id'], buy_request_data, self.prod_environment, method="POST")
                ...
                if shipment_result:
                    ...
                    for shipment in shipment_result:
                        ...
                        form_url = shipment.get('forms') and shipment.get('forms')[0].get('form_url', False)
                        if form_url:
                            form_binary_data = requests.get(form_url).content
                        else:
                            form_binary_data = False
                        ...
                        if form_binary_data:
                            attachments.append(('Easypost_Commercial_Invoice-%s.%s' % (order.name, self.ep_label_file_type), form_binary_data))
                ...
                message = picking.message_post(body=msg, attachments=attachments)
                ##################
                # Modif Captivea #
                ##################
                picking_mod = picking
                picking_mod['carrier_tracking_ref'] = ",".join(tracking_number_list)
                self.cap_generate_shipping_label(picking_mod,message)
                ## FIN Modif #####
    ...
    # Modif Captivea #
    ##################
    def cap_generate_shipping_label(self, picking,message):
        picking._create_shipping_label(message)
    ## FIN Modif #####