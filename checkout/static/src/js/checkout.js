...
//                  Recompute bagde 'number of products in cart' and delivery cost
            var sum = 0;
            $("input[class *= 'js_quantity']").each(function(){
                sum += +$(this).val();
            });
            $(".my_cart_quantity").text(sum);
            $('.o_delivery_carrier_select').each(function() {
                    var $span = $(this).find('span.o_wsale_delivery_badge_price');
                    var $input_carrier = $(this).find('input[name="delivery_type"]');
                    $span.html('<span class="fa fa-spinner fa-spin"></span>');
                    ajax.jsonRpc('/shop/carrier_rate_shipment', 'call', {
                        'carrier_id': $input_carrier.val(),
                    }).then(function (data) {
                        if (data.error_message == false) {
                            $span.html(data.new_amount_delivery);
                        } else {
                           $span.html('<span class="alert">Not available</span>'); 
                        }
